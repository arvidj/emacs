...
(defun glci-end-of-defun ()
  ...
  (when (re-search-forward
       ...
       nil t
       (if (looking-at glci-top-level-definition-re)
           2
         1))
...
(setq glcisk-path "/home/arvid/dev/nomadic-labs/glci-swiss-army-knife/_build/default/bin/main.exe")
...
(defun glcisk-get-token ()
  ...
  (secrets-get-secret
   "login"
   "gitlab access-tokens: glcisk (api)"))
...
(defun glcisk-focus-raw (ci-file job buffer)
  ...
  (let* ((cmd (string-join
               (list
                glcisk-path
                "--scalar-style" "plain"
                "--gl-access-token" (glcisk-get-token)
                "--ci-file" ci-file
                "focus" job
                "--no-preamble")  " ")))
...
(defun glcisk-focus ()
  ...
  (let* ((ci-file (glci-find-ci-file))
         ...
         (b (get-buffer-create (concat "*glcisk focus: " current-job "*")))
        )
...
(defun glcisk-visualize-pipeline ()
  ...
  (interactive)
  (let* ((ci-file (glci-find-ci-file))
         (b (get-buffer-create (concat "*glcisk visualize pipeline*"))))
...
(use-package gitlab-ci-mode
  :ensure t
  :config
  (add-to-list 'auto-mode-alist '("\\.gitlab/.*.yml\\'" . gitlab-ci-mode))
