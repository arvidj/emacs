(use-package gitlab :commands perform-gitlab-request :ensure t)
...
(setq
 ...
 (secrets-get-secret
  "login" "gitlab access-tokens: arvid-gitlab.el (read_api)")
...
ISSUE-ID : The ID of a project issue"
  (s-concat
   "projects/"
   (url-hexify-string (format "%s" project-id))
   "/merge_requests/"
   mr-id))
...
ISSUE-ID : The ID of a project issue"
  (perform-gitlab-request
   ...
   (gitlab--get-merge-request-uri
    (url-hexify-string (format "%s" project-id)) (format "%s" mr-id))
   nil 200))
...
(defun aj/laboxy-go-to-heading (heading-title)
  ...
  (while (and (not
               (string= (arvid-wl/org-heading-title) heading-title))
...
(defun aj/gitlab-start-review (mr-id)
  ...
  (let* ((mr
          (gitlab-get-merge-request aj/gitlab-tezos-project-id mr-id))
    ...
    (if-let ((review-point (aj/laboxy-find-review-heading mr-id)))
      (progn
        ...
        (org-clock-in)
        (message "Clocked into existing review heading: %s"
      ...
      (previous-line)
      (org-clock-in)
      (message "Clocked into new review heading: %s"
               (org-display-outline-path nil t)))))
