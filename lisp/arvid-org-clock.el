...
(defun aj/org-clock-in-out ()
  ...
  (let* ((now-string (format-time-string "%H:%M" (current-time)))
         (at-time-string (read-string "Clock out at time: " (concat now-string " -- " now-string))))
    (if (string-match "^\\([[:digit:]]+\\):\\([[:digit:]]+\\) -- \\([[:digit:]]+\\):\\([[:digit:]]+\\)$" at-time-string)
        (let ((in-time (aj/time-today-at (string-to-number (match-string 1 at-time-string)) (string-to-number (match-string 2 at-time-string))))
              (out-time (aj/time-today-at (string-to-number (match-string 3 at-time-string)) (string-to-number (match-string 4 at-time-string)))))
...
(defun aj/parse-time (at-time-string)
  (if (string-match "^\\([[:digit:]]+\\):\\([[:digit:]]+\\)$" at-time-string)
      (let ((my-at-time
             (aj/time-today-at
              (string-to-number (match-string 1 at-time-string))
              (string-to-number (match-string 2 at-time-string)))))
        my-at-time)
    (error "Expected string on format HH:MM, not: %s" at-time-string)))

(aj/format-time (aj/parse-time "11:30"))

(defun aj/read-time (prompt)
   ""
   (let* ((at-time-string (read-string prompt (format-time-string "%H:%M" (current-time)))))
     (if (string-match "^\\([[:digit:]]+\\):\\([[:digit:]]+\\)$" at-time-string)
         (let ((my-at-time
                (aj/time-today-at
                 (string-to-number (match-string 1 at-time-string))
