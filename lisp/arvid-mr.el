(defun arvid-mr/query-firefox-history (query)
  ...
  (let*
      ((db
        (string-trim
         (shell-command-to-string
          "find $HOME/.mozilla/firefox/ -name places.sqlite -printf \"%T@ %Tc %p\\n\" | sort -n | head -n 1 | cut -d ' ' -f7")))
    ...
    (cl-assert
     (and (file-exists-p dbtmp)
          (> (file-attribute-size (file-attributes dbtmp)) 0)))
...
(defun arvid-mr/issues-get-candidates-firefox ()
  ...
  (let*
      ((limit 1000)
       (query
        (format
         "select p.title, p.url from moz_historyvisits as h, moz_places as p where p.id == h.place_id and p.url like '%%issues%%' order by h.visit_date desc limit %d;"
         limit)))
...
(defun arvid-mr/mrs-get-candidates-firefox ()
  ...
  (let*
      ((limit 1000)
       (query
        (format
         "select p.title, p.url from moz_historyvisits as h, moz_places as p where p.id == h.place_id and p.url like '%%merge_requests%%' order by h.visit_date desc limit %d;"
         limit)))
    (arvid-mr/query-firefox-history query)))
...
(defun arvid-mr/mr-make-url-aux (org repo number)
  ""
  (format "https://gitlab.com/%s/%s/merge_requests/%d"
          org
          repo
          number))
...
(cl-assert
 (equal
  (arvid-mr/issue-parse-gitlab-url
   "https://gitlab.com/nomadic-labs/tezos/issues/29")
  (make-issue
   :title nil
   :org "nomadic-labs"
   :repo "tezos"
   :number 29
   :url "https://gitlab.com/nomadic-labs/tezos/issues/29")))
...
(cl-assert
 (equal
  (arvid-mr/parse-candidate
   '("foo" "https://gitlab.com/tezos/tezos/merge_requests/1887"))
  (make-mergerequest
   :title "foo"
   :org "tezos"
   :repo "tezos"
...
(cl-assert
 (equal
  (arvid-mr/parse-candidate
   '("foo" "https://gitlab.com/tezos/tezos/issues/1887"))
  (make-issue
   :title "foo"
   :org "tezos"
   :repo "tezos"
...
(defvar arvid-mr/shortlink-regexp
  "\\(\\([[:alnum:]_-]+\\)/\\)?\\([[:alnum:]_-]+\\)?\\([!\\#]\\)\\([[:digit:]]*\\)")


(defvar arvid-mr/shortlink-regexp-3
  "\\([[:alnum:]_-]+\\)/\\([[:alnum:]_-]+\\)\\([!\\#]\\)\\([[:digit:]]*\\)")

(defvar arvid-mr/shortlink-regexp-2
  "\\([[:alnum:]_-]+\\)\\([!\\#]\\)\\([[:digit:]]*\\)")

(defvar arvid-mr/shortlink-regexp-1 "\\([!\\#]\\)\\([[:digit:]]*\\)")
...
(cl-assert
 (eql (arvid-mr/type-string-to-type "!") 'arvid-mr/mergerequest))

(defun arvid-mr/parse-shortlink (shortlink)
  (when (and shortlink
             (string-match arvid-mr/shortlink-regexp shortlink))
    (let ((type
           (arvid-mr/type-string-to-type (match-string 4 shortlink)))
      ...
      (pcase type
        ('arvid-mr/mergerequest
         (make-mergerequest
          ...
          :number number
          :url
          (arvid-mr/mr-make-url-aux org repo number)))
        ('arvid-mr/issue
         (make-issue
          :title nil
          :org org
          ...
          :number number
          :url
          (format (arvid-mr/issue-make-url-aux org repo number)
                  org repo number)))))))

(cl-assert
 (equal
  (arvid-mr/parse-shortlink "foo/bar!1123")
  (make-mergerequest
   :org "foo"
   :repo "bar"
   :number 1123
   :url "https://gitlab.com/foo/bar/merge_requests/1123")))
(cl-assert
 (equal
  (arvid-mr/parse-shortlink "foo/tezos!123123")
  (make-mergerequest
   :org "foo"
   :repo "tezos"
   :number 123123
   :url "https://gitlab.com/foo/tezos/merge_requests/123123")))
(cl-assert
 (equal
  (arvid-mr/parse-shortlink
   "https://gitlab.com/nomadic-labs/albert-lang.io/-/jobs/605404211")
  nil))
...
(cl-assert
 (equal
  ...
  (make-issue
   ...
   :repo "bar"
   :number 1123
   :url "https://gitlab.com/foo/bar/issues/1123")))
(cl-assert
 (equal
  (arvid-mr/parse-shortlink "foo/tezos#123123")
  (make-issue
   ...
   :repo "tezos"
   :number 123123
   :url "https://gitlab.com/foo/tezos/issues/123123")))
(cl-assert
 (equal
  (arvid-mr/parse-shortlink
   "https://gitlab.com/nomadic-labs/albert-lang.io/-/jobs/605404211")
...
(defun arvid-mr/mergerequest-make-url (mr-candidate)
  ...
  (interactive)
  (when-let (mr
             (arvid-mr/parse-candidate mr-candidate))
...
(defun arvid-mr/make-shortlink-aux (symbol org repo number full)
  ...
  (cond
   ...
   ((and (not full) (string= org arvid-mr/default-org))
    (format "%s%s%d" repo symbol nr))
   (t
    (format "%s/%s%s%d" org repo symbol nr))))
...
(cl-assert
 (equal
  "nomadic-labs/tezos#23"
  (arvid-mr/issue-make-shortlink
   (arvid-mr/issue-parse-gitlab-url
    "https://gitlab.com/nomadic-labs/tezos/-/issues/23"))))
...
(cl-assert
 (equal
  "nomadic-labs/tezos!1887"
  (arvid-mr/mergerequest-make-shortlink
   (arvid-mr/mr-parse-gitlab-url
    "https://gitlab.com/nomadic-labs/tezos/-/merge_requests/1887"))))
...
(cl-assert
 (equal
  "nomadic-labs/tezos#32"
  (arvid-mr/obj-make-shortlink
   (arvid-mr/parse-gitlab-url
    "https://gitlab.com/nomadic-labs/tezos/issues/32"))))
...
(defun arvid-mr/obj-insert-shortlink (obj-candidate)
  ""
  (when-let (obj
             (arvid-mr/parse-candidate obj-candidate))
    (insert (arvid-mr/obj-make-shortlink obj))))

(defun arvid-mr/open (mr-candidate)
  ""
  (browse-url
   (mergerequest-url (arvid-mr/parse-candidate mr-candidate))))
...
(defun arvid-mr/insert-url (mr-candidate)
  ""
  (insert
   (arvid-mr/mergerequest-make-org-link
    (arvid-mr/parse-candidate mr-candidate))))

(defun arvid-mr/insert-title (mr-candidate)
  ""
  (insert
   (mergerequest-title (arvid-mr/parse-candidate mr-candidate))))
...
(defun arvid-mr/helm-mrs ()
  ...
  (helm
   ...
   (helm-build-sync-source
    "MRs visited in Firefox"
    :candidates (arvid-mr/mrs-get-candidates-firefox)
    :action
    '(("Insert shortlink format (org/repo!no)"
       .
       arvid-mr/obj-insert-shortlink)
...
(defun arvid-mr/shortlink-at-point ()
  ...
  (when (or (thing-at-point-looking-at
             arvid-mr/shortlink-regexp-3 500)
            (thing-at-point-looking-at
             arvid-mr/shortlink-regexp-2 500)
            (thing-at-point-looking-at
             arvid-mr/shortlink-regexp-1 500))
...
(defun arvid-mr/org-open-gitlab-shortlink ()
  ...
  (let ((atp (arvid-mr/shortlink-at-point)))
    (when-let (obj
               (arvid-mr/parse-shortlink atp))
      (browse-url (arvid-mr/obj-url obj))
      t)))

(add-hook
 'org-open-at-point-functions 'arvid-mr/org-open-gitlab-shortlink)
...
(arvid-mr/gitlab-shortlink-description
 "https://gitlab.com/tezos/tezos/-/merge_requests/1887" "Foo")
...
(defun arvid-gl/gitlab-goto-line ()
  ...
  (let ((repo "tezos/tezos")
    ...
    (browse-url
     (format "https://gitlab.com/%s/-/blob/%s/%s#L%d"
             repo
             branch
             file
             line))))
