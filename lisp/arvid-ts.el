...
(defun find-typo3-site-root-dir (&optional root)
  "find-typo3-root-dir traverses the file tree upwards until an
typo3-root dir is found. If it is found, it is returned,
otherwise nil."
  (let ((root (or root default-directory)))
	(cond
	 ;; Is this root?
...
(defun get-ts-path-at-point () 
  ""
  (interactive)
  (save-excursion
	;; Go to first non-blank-link
	(setq path "")
	(do )
	...
	(while t
	  ...
	  (setq path 
			(concat (buffer-substring-no-properties
					...
					path))
	  (when (eq= (progn (back-to-indentation) (point))
