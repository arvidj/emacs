...
(defcustom arvid-wl/worklog-file
...
(defun arvid-wl/org-remove-drawer-from-heading ()
  ...
  (save-excursion
    (cl-assert
     ...
     nil
     "arvid-wl/org-remove-drawer-from-heading: must be called on an outline heading")
    (forward-line)
    (if (org-at-drawer-p)
...
(defun arvid-wl/heading-to-list-item (time-range time-range-prop)
  ...
  (let ((time (or (get-text-property (point) time-range-prop) 0))
    ...
    (cl-assert
     todo nil
     (format "arvid-wl/heading-to-list-item: missing TODO state in %s"
             title))
    (org-cut-subtree)
    (insert
     (format " - %s %s <%sh%02d>\n"
...
(defun arvid-wl/heading-has-leaves-p ()
  ...
  (cl-assert
   (outline-on-heading-p)
   nil
   "arvid-wl/heading-has-leaves-p: must be called on an outline heading")
  (save-excursion
    (< (org-current-level)
       (progn
...
(cl-assert
 (equal (arvid-wl/workitem-info "Foo (sd#bar)") '("Foo" . "sd#bar")))
...
(defun arvid-wl/heading-add-time-suffix (total-time)
  (cl-assert
   (outline-on-heading-p)
   nil
   "arvid-wl/heading-add-time-suffix: must be called on an outline heading")
  (save-excursion
    (end-of-line)
...
(defun arvid-wl/prune2-composed-workitem (time-range time-range-prop)
  (cl-assert
   (outline-on-heading-p)
   nil
   "arvid-wl/prune2-composed-workitem: must be called on an outline heading")

  ;; (org-clock-display time-range)
  (let* ((init-level (org-current-level))
         (workitem-info
          (arvid-wl/workitem-info (arvid-wl/org-heading-title)))
         (workitem-title (car workitem-info))
         (workitem-issue (cdr workitem-info))
         (total-time
          (or (get-text-property (point) time-range-prop) 0))
         (subheading-time 0))
                 ...
                 (progn
                   (cond
                    ...
                    ((string= (org-entry-get (point) "WORKITEM") "t")
                     ...
                     (arvid-wl/prune2-composed-workitem
                      time-range time-range-prop))
                    ;; A simple work item
                    ((not (arvid-wl/heading-has-leaves-p))
                     (arvid-wl/heading-to-list-item
                      time-range time-range-prop))
                    ;; A category
                    (t
...
(defun arvid-wl/get-week-start-from-relative (week &optional time)
  ""
  (let* ((weekday
          (mod (- (decoded-time-weekday (decode-time time)) 1) 7))
         (delta
          (make-decoded-time
           :day
           (- (+ weekday (case week ('thisweek 0) ('lastweek 7)))))))
    (print weekday)
...
(cl-assert
 (string=
  "2021w01"
  (format-time-string "%Yw%V"
                      (arvid-wl/get-week-start-from-relative
                       'thisweek
                       (date-to-time "2021-01-10 00:00")))))
(cl-assert
 (string=
  "2021w02"
  (format-time-string "%Yw%V"
...
(defun arvid-wl/spinoff-filename (week &optional time)
  ...
  (let* ((delta
          (make-decoded-time
           :day (case week ('thisweek 0) ('lastweek -7))))
         (time
          (encode-time (decoded-time-add (decode-time time) delta))))
...
(cl-assert
 (string=
  (arvid-wl/spinoff-filename 'thisweek
                             (date-to-time "2020-04-15 00:00"))
  "2020w16.org"))
(cl-assert
 (string=
  (arvid-wl/spinoff-filename 'lastweek
                             (date-to-time "2020-04-15 00:00"))
  "2020w15.org"))
...
(defun arvid-wl/decode-buffer-name (file-name)
  ""
  (if (string-match
       "\\([[:digit:]]+\\)w\\([[:digit:]]+\\).org" file-name)
      (let ((year (string-to-number (match-string 1 file-name)))
            (week (string-to-number (match-string 2 file-name))))
        (cond
         ((string=
           (format "%sw%s.org" year week)
           (format-time-string "%Yw%V.org"))
          'thisweek)
         ((string=
           (format "%sw%s.org" year (+ week 1))
           (format-time-string "%Yw%V.org"))
          'lastweek)))))
...
(defun arvid-wl/worklog-check-sanity (week)
  ...
  (interactive (list
                (intern
                 (ido-completing-read
                  "Spin-off work log for which week? "
                  '("thisweek" "lastweek")
                  nil
                  t))))
  ...
  (let ((time-range-prop :org-clock-minutes-default)
    ...
    (if (not messages)
      ...
      (with-current-buffer (get-buffer-create
                            "*arvid-worklog/sanity*")
        (display-buffer (current-buffer))

        (insert
         (format "Sanity check in worklog file %s\n\n"
...
(defun arvid-wl/org-clock-in-hook-no-workitem ()
  ...
  (cl-assert
   (not (string= (org-entry-get (point) "WORKITEM") "t"))
   nil
...
(defun arvid-wl/overview2 ()
  ...
  (interactive (list
                (intern
                 (ido-completing-read
                  "Overview work log for which week? "
                  '("thisweek" "lastweek")
                  nil
                  t))))
  ...
  (save-excursion
    ...
    (let ((overview-buffer
           (get-buffer-create "*arvid-worklog/overview*"))
          (worklog-buffer (current-buffer)))


      (let
          ((today (format-time-string "CLOCK: [%Y-%m-%d"))
           (re (regexp-quote today))
...
(defun arvid-wl/magit-commit-msg-add-week-no ()
  ...
  (if nil
      (insert
       (format-time-string
        "%Yw%V"
        (arvid-wl/get-week-start-from-relative 'thisweek)))))
