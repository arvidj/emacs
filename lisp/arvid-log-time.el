...
(defcustom org-camp-auth-token
  "2a47c606253624fa03635ef58d6d112a8b0d2c08"
...
specified by `org-camp-target-file'."
  (with-temp-file org-camp-target-file
    ...
    (let ((xml-nodes
           (xml-parse-region 1 (buffer-size buffer) buffer)))
...
provided `request' string."
  (cond
   ((= (length org-camp-subdomain) 0)
    (error "The value of `org-camp-subdomain' must be customized"))
   (t
    (concat
     "https://" org-camp-subdomain ".basecamphq.com/" request))))
...
(defun org-camp-pull ()
  ...
  (save-excursion
    (let ((url-request-method "GET")
      ...
      (url-retrieve
       (org-camp-url "todo_lists.xml") 'org-camp-todo-url-callback))))

(defun org-camp-url-retrieve
    (method request callback &optional post-data)
  (if (= (length org-camp-auth-token) 0)
      (error "The `org-camp-auth-token' must be customized"))
           ...
           ("Authorization" .
            ,(concat
              ...
              (base64-encode-string
               (concat org-camp-auth-token ":X")))))))
    (url-retrieve
     (org-camp-url request) 'org-camp-parse-xml-cb (list callback))))
...
(defun org-camp-log-time (project-id person-id date hours description)
  ...
  (org-camp-url-retrieve
   "POST"
   (concat
    "projects/" project-id "/time_entries.xml")
   'org-camp-log-time-callback
   (format
    "<time-entry> <person-id>%d</person-id> <date>%s</date> <hours>%f</hours> <description>%s</description> </time-entry>"
    person-id date hours description)))
...
(defun org-camp-parse-xml-cb (status callback)
  (funcall callback
           status
           (car (xml-parse-region 1 (buffer-size (current-buffer))))))

(defun org-camp-get-projects (callback)
  ""
  (interactive)
  (org-camp-url-retrieve "GET" "projects.xml" 'org-camp-parse-xml-cb
                         callback))
...
(defun org-camp-message-my-id-callback (status xml-nodes)
  ...
  (print
   (string-to-number
    (car (cddr (car (xml-get-children xml-nodes 'id)))))))
...
;; (defun org-camp-browse-todo ()
;;   "Open the current issue in external browser."
;;   (interactive)
;;   (ensure-on-issue
;;    (url-browse (concat "http://" org-camp-subdomain "basecamphq.com" (org-jira-id)))))

;; (org-add-link-type "bc-todo" 'org-camp--open)
